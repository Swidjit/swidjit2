class SubscriptionsController < ApplicationController
  ...
  def index
    ...
    @items = Item.where('user_id in (?)',user_ids)
    render 'items/index'
  end
  ...
  def create
    if params[:user_id].present?
      ...
      @subscription = Subscription.new(:subscription_id => params[:user_id],:subscription_type => params[:subscription_type], :user => current_user)
